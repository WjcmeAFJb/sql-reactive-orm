...
export function SqlConsole() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex w-[680px] max-w-[calc(100vw-2rem)] max-h-[80vh] flex-col rounded-lg border border-[--color-border] bg-background shadow-2xl">
      <ConsoleHeader />
      <Samples state={s} />
      <Editor state={s} textareaRef={textareaRef} onRun={run} />
      <ConsoleFooter state={s} onRun={run} />
      <ResultView state={s} />
    </div>
  );
}
...
function Samples({ state }: { state: ConsoleState }) {
  return (
...
function Editor({
  ...
  function onKeyDown(e: KeyboardEvent<HTMLTextAreaElement>): void {
    ...
    if (e.key === "Tab") {
      ...
      state.sql = state.sql.slice(0, start) + "  " + state.sql.slice(end);
      queueMicrotask(() => {
  ...
  return (
    <textarea
      ref={textareaRef}
      value={state.sql}
      onChange={(e) => (state.sql = e.target.value)}
      onKeyDown={onKeyDown}
      spellCheck={false}
...
function ResultView({ state }: { state: ConsoleState }) {
  ...
  if (result.kind === "ok") {
    return (
      <div className="min-h-20 overflow-auto border-t border-[--color-border] px-3 py-3 text-xs">
  ...
  if (rows.length === 0) {
    return (
      <div className="min-h-20 overflow-auto border-t border-[--color-border] px-3 py-3 text-xs text-muted-foreground">
  ...
  return (
    <div className="min-h-20 overflow-auto border-t border-[--color-border] max-h-[40vh]">
